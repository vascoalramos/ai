...
class STRIPS(SearchDomain):
    ...
    # ( returns [], if the action is not applicable in the state)
    def result(self, state, action):
        if action.__repr__() not in [action_i.__repr__() for action_i in self.actions(state)]:
            return None

        for result_action in action.neg:
            state.remove(result_action)
        
        for result_action in action.pos:
            state.append(result_action)
